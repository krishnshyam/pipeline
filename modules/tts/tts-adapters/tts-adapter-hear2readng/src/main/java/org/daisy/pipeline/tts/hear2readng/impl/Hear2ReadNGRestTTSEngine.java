...
/**
 * Connector class to synthesize audio using the hear2readng tts engine.
 *
 * <p>This connector is based on their REST Api.</p>
 *
 * This has been heavily adapted from the existing code for Google TTS
 *
 * @author krishnshyam @ gmail.com
 */
public class Hear2ReadNGRestTTSEngine extends TTSEngine {

	private final AudioFormat mAudioFormat;
	private final Scheduler<Schedulable> mRequestScheduler;
	private final int mPriority;
	private final Hear2ReadNGRequestBuilder mRequestBuilder;
	private final float mSpeechRate;

	private static final URL ssmlTransformer = URLs.getResourceFromJAR("/transform-ssml.xsl", Hear2ReadNGRestTTSEngine.class);
	private static final Logger logger = LoggerFactory.getLogger(Hear2ReadNGRestTTSEngine.class);

	// TODO remove audioFormat -shyam
	public Hear2ReadNGRestTTSEngine(Hear2ReadNGTTSService hear2readngService, String serverAddress, AudioFormat audioFormat, int priority,
	                           float speechRate) {
		super(hear2readngService);
		mPriority = priority;
		mAudioFormat = audioFormat;
		mSpeechRate = speechRate;
		mRequestScheduler = new ExponentialBackoffScheduler<Schedulable>();
		mRequestBuilder = new Hear2ReadNGRequestBuilder(serverAddress);
	}
	...
	public SynthesisResult synthesize(XdmNode ssml, Voice voice, TTSResource threadResources)
		...
		if (voice != null) {
			...
			languageCode = voice.getLocale().iterator().next().toString().split("-")[0];
		} else {
		...
		try {
			Request<JSONObject> speechRequest = mRequestBuilder.newRequest()
				.withSpeechRate(speechRate)
				.withAction(Hear2ReadNGRestAction.SPEECH)
				.withLanguageCode(languageCode)
				.withVoice(name)
				.withText(sentence)
				.build();
			ArrayList<byte[]> result = new ArrayList<>();
			//TODO these responses don't exist in our API -shyam
	...
	public Collection<Voice> getAvailableVoices() throws SynthesisException, InterruptedException {
		...
		try {
			Request<JSONObject> voicesRequest = mRequestBuilder.newRequest()
				.withAction(Hear2ReadNGRestAction.VOICES)
				.build();
			mRequestScheduler.launch(() -> {
